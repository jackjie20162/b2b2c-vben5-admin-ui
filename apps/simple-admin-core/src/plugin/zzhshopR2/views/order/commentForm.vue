...
import { commentFormSchemas } from './schema';

defineOptions({
  name: 'ZzhshopR2CommentForm',
});
...
async function onSubmit(values: Record<string, any>) {
  const payload = { ...values } as CommentInfo;
...
const [Form, formApi] = useVbenForm({
  ...
  schema: [...(commentFormSchemas.schema as any)],
...
const [Modal, modalApi] = useVbenModal({
  ...
  onOpenChange(isOpen: boolean) {
    ...
    modalApi.setState({
      title: isUpdate.value ? '编辑评论' : '添加评论',
    });
  },
});
