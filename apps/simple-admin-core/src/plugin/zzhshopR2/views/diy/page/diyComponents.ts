 ...
 */
export interface DiyComponentDef {
  name: string;
  type: string;
  icon: string;
...
export interface DiyGroup {
  key: string;
  label: string;
  items: DiyComponentDef[];
}

export const diyGroups: DiyGroup[] = [
  {
    key: 'Basics',
    label: '基础组件',
    items: [
...
export const moduleStyleCatalog: Record<string, string> = {
  height: '高度（px）',
  width: '宽度（%）',
...
export const dataKeyLabelMap: Record<string, string> = {
  image: '图片',
  iconImage: '菜单图片',
  icon: '图标类名',
  categoryIcon: '图标类名',
...
/** params 字段中文名映射 */
export const paramsKeyLabelMap: Record<string, string> = {
  interval: '轮播间隔（ms）',
  height: '高度',
  banstyle: '轮播样式',
  imgLayout: '布局方式',
...
/** 深拷贝组件默认模板，作为新组件加入页面 */
export function cloneComponentDef(def: DiyComponentDef) {
