...
const columns = [
  ...
  { title: '收货人', dataIndex: 'name', width: 110 },
  { title: '手机号', dataIndex: 'mobile', width: 130 },
  { title: '默认', dataIndex: 'is_default', width: 80 },
  { title: '所在地区', dataIndex: 'region', minWidth: 180, ellipsis: true },
  { title: '详细地址', dataIndex: 'address', minWidth: 160, ellipsis: true },
  { title: '经纬度', dataIndex: 'location', width: 140 },
...
function openForm(record?: AddressInfo) {
...
function handleDelete(ids: number[]) {
  Modal.confirm({
    title: '删除地址',
    content: `确认删除选中的 ${ids.length} 条地址？删除后不可恢复`,
    okType: 'danger',
    onOk: async () => {
  ...
  <Page auto-content-height>
    ...
    <div class="list-panel">
      <div class="page-head">
        ...
        <div class="page-sub">用于监管用户地址</div>
      </div>
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.dataIndex === 'is_default'">
            <Tag v-if="record.is_default === '1'" color="green">默认</Tag>
            <span v-else>-</span>
          </template>
