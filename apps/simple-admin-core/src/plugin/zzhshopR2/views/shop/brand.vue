...
const columns = [
  ...
  { title: '品牌名', dataIndex: 'name', minWidth: 160, ellipsis: true },
  { title: '图片', dataIndex: 'image', width: 80 },
  { title: '创建时间', dataIndex: 'createtime', width: 170 },
  { title: '更新时间', dataIndex: 'updatetime', width: 170 },
  { title: '状态值', dataIndex: 'state', width: 100 },
...
function openForm(record?: BrandInfo) {
...
function handleDelete(ids: number[]) {
  Modal.confirm({
    title: '删除品牌',
    content: `确认删除选中的 ${ids.length} 个品牌？删除后不可恢复`,
    okType: 'danger',
    onOk: async () => {
  ...
  <Page auto-content-height>
    ...
    <div class="list-panel">
      <div class="page-head">
        ...
        <div class="page-sub">用于管理平台品牌及其审核状态</div>
      </div>
      ...
      >
        <TabPane key="" tab="全部" />
        <TabPane key="0" tab="审核中" />
        <TabPane key="1" tab="已审核" />
      </Tabs>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'state'">
            <Tag :color="record.state === '1' ? 'green' : 'orange'">
              {{ record.state === '1' ? '已审核' : '审核中' }}
            </Tag>
          </template>
